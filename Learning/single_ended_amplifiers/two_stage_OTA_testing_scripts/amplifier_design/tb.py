...
if simulation == 1:
    ...
    sim_comands.write_param(netlist,"VCM",str(VCM))
    ...
    size = int(VCM * 100)
    offset = data.iloc[size,1] - data.iloc[size,0]
    print(offset)
    
    
    offset = simulation_data_processing.value_converter_to_string(offset)
    power = simulation_data_processing.value_converter_to_string(data.iloc[size,3]*(VDD-VSS))
...
if simulation == 2:
    ...
    for current_corner in corners:
        ...
        sim_comands.write_param(AC_sch_path,"VCM",str(VCM))
