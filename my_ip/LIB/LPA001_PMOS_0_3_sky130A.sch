v {xschem version=3.4.5 file_version=1.2
}
G {}
K {}
V {}
S {}
E {}
L 4 1700 -440 2040 -440 {}
L 4 2040 -440 2040 -400 {}
L 4 1700 -400 2040 -400 {}
L 4 1700 -440 1700 -400 {}
L 4 1860 -440 1860 -400 {}
L 4 1700 -400 2040 -400 {}
L 4 2040 -400 2040 -360 {}
L 4 1700 -360 2040 -360 {}
L 4 1700 -400 1700 -360 {}
L 4 1860 -400 1860 -360 {}
L 4 1700 -360 2040 -360 {}
L 4 2040 -360 2040 -320 {}
L 4 1700 -320 2040 -320 {}
L 4 1700 -360 1700 -320 {}
L 4 1860 -360 1860 -320 {}
L 4 1700 -320 2040 -320 {}
L 4 2040 -320 2040 -280 {}
L 4 1700 -280 2040 -280 {}
L 4 1700 -320 1700 -280 {}
L 4 1860 -320 1860 -280 {}
L 4 1700 -280 2040 -280 {}
L 4 1700 -280 2040 -280 {}
L 4 2040 -280 2040 -240 {}
L 4 1700 -240 2040 -240 {}
L 4 1700 -280 1700 -240 {}
L 4 1860 -280 1860 -240 {}
L 4 1700 -240 2040 -240 {}
L 4 2040 -240 2040 -200 {}
L 4 1700 -200 2040 -200 {}
L 4 1700 -240 1700 -200 {}
L 4 1860 -240 1860 -200 {}
L 4 1700 -200 2040 -200 {}
L 4 2040 -200 2040 -160 {}
L 4 1700 -160 2040 -160 {}
L 4 1700 -200 1700 -160 {}
L 4 1860 -200 1860 -160 {}
L 4 1700 -160 2040 -160 {}
L 4 2040 -160 2040 -120 {}
L 4 1700 -120 2040 -120 {}
L 4 1700 -160 1700 -120 {}
L 4 1860 -160 1860 -120 {}
L 4 1700 -120 2040 -120 {}
L 4 2040 -120 2040 -80 {}
L 4 1700 -80 2040 -80 {}
L 4 1700 -120 1700 -80 {}
L 4 1860 -120 1860 -80 {}
L 4 1700 -80 2040 -80 {}
L 4 1700 -80 2040 -80 {}
L 4 2040 -80 2040 -40 {}
L 4 1700 -40 2040 -40 {}
L 4 1700 -80 1700 -40 {}
L 4 1860 -80 1860 -40 {}
L 4 1700 -40 1700 0 {}
L 4 2040 -40 2040 0 {}
L 4 1700 0 2040 0 {}
L 4 1860 -40 1860 0 {}
L 4 1700 0 1700 40 {}
L 4 2040 0 2040 40 {}
L 4 1860 0 1860 40 {}
L 4 1700 40 2040 40 {}
T {specifications tt 27 Cº degrees} 1690 -530 0 0 1 1 {}
T {UGBW} 1740 -430 0 0 0.4 0.4 {}
T {7.3 kHz} 1920 -430 0 0 0.4 0.4 {}
T {CMRR} 1740 -390 0 0 0.4 0.4 {}
T {39 db} 1920 -390 0 0 0.4 0.4 {}
T {dc gain} 1740 -350 0 0 0.4 0.4 {}
T {34 db} 1920 -350 0 0 0.4 0.4 {}
T {ICMR} 1740 -310 0 0 0.4 0.4 {}
T {} 1920 -310 0 0 0.4 0.4 {}
T {0 V - 0.3 V} 1900 -310 0 0 0.4 0.4 {}
T {OUTPUT range} 1710 -270 0 0 0.35 0.35 {}
T {} 2120 -290 0 0 0.4 0.4 {}
T {0.05 V - 0.25 V} 1870 -270 0 0 0.4 0.4 {}
T {phase margin} 1710 -230 0 0 0.4 0.4 {}
T {60º} 1930 -230 0 0 0.4 0.4 {}
T {PSSR+} 1740 -190 0 0 0.4 0.4 {}
T {11 db} 1920 -190 0 0 0.4 0.4 {}
T {11 db} 1920 -150 0 0 0.4 0.4 {}
T {offset at 900mV} 1710 -110 0 0 0.3 0.3 {}
T {Power} 1750 -70 0 0 0.35 0.35 {}
T {PSSR-} 1740 -150 0 0 0.4 0.4 {}
T {51 uV} 1920 -110 0 0 0.4 0.4 {}
T {710 nW} 1920 -70 0 0 0.4 0.4 {}
T {VDD} 1750 -30 0 0 0.35 0.35 {}
T {0.3 V} 1920 -10 2 1 0.4 0.4 {}
T {Cp} 1750 10 0 0 0.35 0.35 {}
T {30p F} 1920 30 2 1 0.4 0.4 {}
T {Based this paper:
Design and Simulation of Operational Amplifier
using Non Conventional Method for Low Power
Applications} 750 -1100 0 0 1 1 {}
N -370 -770 -300 -770 {
lab=VDD}
N -370 -730 -300 -730 {
lab=VSS}
N -370 -690 -300 -690 {
lab=VIN-}
N -370 -650 -300 -650 {
lab=VIN+}
N -370 -610 -300 -610 {
lab=VOUT}
N 240 -210 240 -180 {
lab=VSS}
N 240 -560 240 -240 {
lab=VB1}
N 240 -620 310 -620 {
lab=VDD}
N 240 -280 300 -280 {
lab=VB1}
N 300 -280 300 -210 {
lab=VB1}
N 280 -210 300 -210 {
lab=VB1}
N 240 -180 240 -60 {
lab=VSS}
N 860 -210 1010 -210 {
lab=VB1}
N 820 -210 820 -180 {
lab=VSS}
N 1050 -210 1050 -180 {
lab=VSS}
N 820 -180 820 -140 {
lab=VSS}
N 820 -140 1050 -140 {
lab=VSS}
N 1050 -180 1050 -140 {
lab=VSS}
N 820 -470 820 -240 {
lab=#net1}
N 1050 -470 1050 -240 {
lab=#net2}
N 820 -600 820 -530 {
lab=VDD}
N 310 -620 1050 -620 {
lab=VDD}
N 820 -620 820 -600 {
lab=VDD}
N 1050 -620 1050 -540 {
lab=VDD}
N 1050 -540 1050 -530 {
lab=VDD}
N 720 -500 780 -500 {
lab=#net1}
N 720 -500 720 -420 {
lab=#net1}
N 720 -420 820 -420 {
lab=#net1}
N 1090 -500 1150 -500 {
lab=#net2}
N 1150 -500 1150 -420 {
lab=#net2}
N 1050 -420 1150 -420 {
lab=#net2}
N 680 -620 680 -530 {
lab=VDD}
N 1190 -620 1190 -530 {
lab=VDD}
N 1050 -620 1190 -620 {
lab=VDD}
N 980 -500 1050 -500 {
lab=VIN-}
N 680 -540 680 -530 {
lab=VDD}
N 610 -500 680 -500 {
lab=VIN-}
N 820 -500 890 -500 {
lab=VIN+}
N 1190 -500 1260 -500 {
lab=VIN+}
N 240 -60 1130 -60 {
lab=VSS}
N 680 -90 680 -60 {
lab=VSS}
N 680 -470 680 -120 {
lab=#net3}
N 680 -140 740 -140 {
lab=#net3}
N 740 -140 740 -90 {
lab=#net3}
N 720 -90 740 -90 {
lab=#net3}
N 1190 -470 1190 -120 {
lab=a}
N 1130 -60 1190 -60 {
lab=VSS}
N 740 -90 1150 -90 {
lab=#net3}
N 1190 -90 1190 -60 {
lab=VSS}
N 1360 -170 1360 -90 {
lab=#net3}
N 1360 -90 1410 -90 {
lab=#net3}
N 1450 -90 1450 -60 {
lab=VSS}
N 1190 -60 1450 -60 {
lab=VSS}
N 1160 -170 1360 -170 {
lab=#net3}
N 1140 -170 1160 -170 {
lab=#net3}
N 1140 -170 1140 -90 {
lab=#net3}
N 1450 -470 1450 -120 {
lab=VOUT}
N 1380 -500 1410 -500 {
lab=a}
N 1380 -500 1380 -410 {
lab=a}
N 1200 -410 1380 -410 {
lab=a}
N 1190 -410 1200 -410 {
lab=a}
N 1190 -620 1450 -620 {
lab=VDD}
N 1450 -620 1450 -530 {
lab=VDD}
N 1450 -530 1450 -500 {
lab=VDD}
C {devices/iopin.sym} -370 -770 0 1 {name=p1 lab=VDD}
C {devices/lab_pin.sym} -300 -770 2 0 {name=p4 sig_type=std_logic lab=VDD}
C {devices/iopin.sym} -370 -730 0 1 {name=p6 lab=VSS}
C {devices/lab_pin.sym} -300 -730 2 0 {name=p7 sig_type=std_logic lab=VSS}
C {devices/iopin.sym} -370 -690 0 1 {name=p12 lab=VIN-}
C {devices/lab_pin.sym} -300 -690 2 0 {name=p13 sig_type=std_logic lab=VIN-}
C {devices/iopin.sym} -370 -650 0 1 {name=p17 lab=VIN+}
C {devices/lab_pin.sym} -300 -650 2 0 {name=p18 sig_type=std_logic lab=VIN+}
C {devices/iopin.sym} -370 -610 0 1 {name=p19 lab=VOUT}
C {devices/lab_pin.sym} -300 -610 2 0 {name=p20 sig_type=std_logic lab=VOUT}
C {sky130_fd_pr/nfet_01v8_lvt_nf.sym} 260 -210 0 1 {name=M33
L=1
W=4
nf=4 mult=2
model=nfet_01v8_lvt
spiceprefix=X
}
C {isource.sym} 240 -590 0 0 {name=I0 value=20n}
C {devices/lab_pin.sym} 280 -620 1 0 {name=p2 sig_type=std_logic lab=VDD}
C {devices/lab_pin.sym} 240 -60 2 0 {name=p3 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 300 -280 2 0 {name=p5 sig_type=std_logic lab=VB1}
C {sky130_fd_pr/nfet_01v8_lvt_nf.sym} 840 -210 0 1 {name=M1
L=1
W=4
nf=4 mult=10
model=nfet_01v8_lvt
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8_lvt_nf.sym} 1030 -210 0 0 {name=M2
L=1
W=4
nf=4 mult=10
model=nfet_01v8_lvt
spiceprefix=X
}
C {devices/lab_pin.sym} 930 -210 1 0 {name=p8 sig_type=std_logic lab=VB1}
C {devices/lab_pin.sym} 930 -140 1 0 {name=p9 sig_type=std_logic lab=VSS}
C {sky130_fd_pr/pfet_01v8_lvt_nf.sym} 1170 -500 0 0 {name=M5
L=1
W=4
nf=6 mult=4
model=pfet_01v8_lvt
spiceprefix=X
}
C {devices/lab_pin.sym} 980 -500 1 1 {name=p10 sig_type=std_logic lab=VIN-}
C {devices/lab_pin.sym} 610 -500 1 1 {name=p11 sig_type=std_logic lab=VIN-}
C {devices/lab_pin.sym} 890 -500 3 0 {name=p14 sig_type=std_logic lab=VIN+}
C {devices/lab_pin.sym} 1260 -500 3 0 {name=p15 sig_type=std_logic lab=VIN+}
C {sky130_fd_pr/nfet_01v8_lvt_nf.sym} 700 -90 0 1 {name=M6
L=1
W=8
nf=10 mult=16
model=nfet_01v8_lvt
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8_lvt_nf.sym} 1170 -90 0 0 {name=M7
L=1
W=8
nf=10 mult=16
model=nfet_01v8_lvt
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8_lvt_nf.sym} 1430 -90 0 0 {name=M8
L=1
W=3.4
nf=22 mult=4
model=nfet_01v8_lvt
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8_lvt_nf.sym} 1430 -500 0 0 {name=M9
L=1
W=4
nf=4 mult=20
model=pfet_01v8_lvt
spiceprefix=X
}
C {devices/lab_pin.sym} 1450 -320 0 0 {name=p16 sig_type=std_logic lab=VOUT}
C {devices/lab_pin.sym} 1190 -320 0 0 {name=p21 sig_type=std_logic lab=a}
C {sky130_fd_pr/pfet_01v8_lvt_nf.sym} 1070 -500 0 1 {name=M3
L=1
W=4
nf=6 mult=4
model=pfet_01v8_lvt
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8_lvt_nf.sym} 700 -500 0 1 {name=M4
L=1
W=4
nf=6 mult=4
model=pfet_01v8_lvt
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8_lvt_nf.sym} 800 -500 0 0 {name=M10
L=1
W=4
nf=6 mult=4
model=pfet_01v8_lvt
spiceprefix=X
}
C {devices/title-3.sym} 10 470 0 0 {name=l2 author="Vasco Luz" rev=1.0 lock=false}
