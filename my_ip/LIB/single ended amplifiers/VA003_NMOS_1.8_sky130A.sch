v {xschem version=3.4.5 file_version=1.2
}
G {}
K {}
V {}
S {}
E {}
L 4 -890 -390 -890 180 {}
L 4 -890 -390 -340 -390 {}
L 4 -890 180 -350 180 {}
L 4 -350 180 -340 180 {}
L 4 -340 -390 -340 180 {}
L 4 1700 -440 2040 -440 {}
L 4 2040 -440 2040 -400 {}
L 4 1700 -400 2040 -400 {}
L 4 1700 -440 1700 -400 {}
L 4 1860 -440 1860 -400 {}
L 4 1700 -400 2040 -400 {}
L 4 2040 -400 2040 -360 {}
L 4 1700 -360 2040 -360 {}
L 4 1700 -400 1700 -360 {}
L 4 1860 -400 1860 -360 {}
L 4 1700 -360 2040 -360 {}
L 4 2040 -360 2040 -320 {}
L 4 1700 -320 2040 -320 {}
L 4 1700 -360 1700 -320 {}
L 4 1860 -360 1860 -320 {}
L 4 1700 -320 2040 -320 {}
L 4 2040 -320 2040 -280 {}
L 4 1700 -280 2040 -280 {}
L 4 1700 -320 1700 -280 {}
L 4 1860 -320 1860 -280 {}
L 4 1700 -280 2040 -280 {}
L 4 1700 -280 2040 -280 {}
L 4 2040 -280 2040 -240 {}
L 4 1700 -240 2040 -240 {}
L 4 1700 -280 1700 -240 {}
L 4 1860 -280 1860 -240 {}
L 4 1700 -240 2040 -240 {}
L 4 2040 -240 2040 -200 {}
L 4 1700 -200 2040 -200 {}
L 4 1700 -240 1700 -200 {}
L 4 1860 -240 1860 -200 {}
L 4 1700 -200 2040 -200 {}
L 4 2040 -200 2040 -160 {}
L 4 1700 -160 2040 -160 {}
L 4 1700 -200 1700 -160 {}
L 4 1860 -200 1860 -160 {}
L 4 1700 -160 2040 -160 {}
L 4 2040 -160 2040 -120 {}
L 4 1700 -120 2040 -120 {}
L 4 1700 -160 1700 -120 {}
L 4 1860 -160 1860 -120 {}
L 4 1700 -120 2040 -120 {}
L 4 2040 -120 2040 -80 {}
L 4 1700 -80 2040 -80 {}
L 4 1700 -120 1700 -80 {}
L 4 1860 -120 1860 -80 {}
L 4 1700 -80 2040 -80 {}
L 4 1700 -80 2040 -80 {}
L 4 2040 -80 2040 -40 {}
L 4 1700 -40 2040 -40 {}
L 4 1700 -80 1700 -40 {}
L 4 1860 -80 1860 -40 {}
L 4 1700 -40 1700 0 {}
L 4 2040 -40 2040 0 {}
L 4 1700 0 2040 0 {}
L 4 1860 -40 1860 0 {}
L 4 1700 0 1700 40 {}
L 4 2040 0 2040 40 {}
L 4 1860 0 1860 40 {}
L 4 1700 40 2040 40 {}
T {self bias generator} -860 -460 0 0 1 1 {}
T {First stage} 10 -830 0 0 1 1 {}
T {specifications tt 27 Cº degrees} 1690 -530 0 0 1 1 {}
T {UGBW} 1740 -430 0 0 0.4 0.4 {}
T {42 Mhz} 1920 -430 0 0 0.4 0.4 {}
T {CMRR} 1740 -390 0 0 0.4 0.4 {}
T {66 db} 1920 -390 0 0 0.4 0.4 {}
T {dc gain} 1740 -350 0 0 0.4 0.4 {}
T {32 db} 1920 -350 0 0 0.4 0.4 {}
T {ICMR} 1740 -310 0 0 0.4 0.4 {}
T {} 1920 -310 0 0 0.4 0.4 {}
T {0.8 V - 1.8 V} 1900 -310 0 0 0.4 0.4 {}
T {OUTPUT range} 1710 -270 0 0 0.35 0.35 {}
T {} 1920 -270 0 0 0.4 0.4 {}
T {0.5 V - 1.6 V} 1890 -270 0 0 0.4 0.4 {}
T {phase margin} 1710 -230 0 0 0.4 0.4 {}
T {56º} 1930 -230 0 0 0.4 0.4 {}
T {PSSR+} 1740 -190 0 0 0.4 0.4 {}
T {31 db} 1920 -190 0 0 0.4 0.4 {}
T {58 db} 1920 -150 0 0 0.4 0.4 {}
T {offset at 900mV} 1710 -110 0 0 0.3 0.3 {}
T {Power} 1750 -70 0 0 0.35 0.35 {}
T {PSSR-} 1740 -150 0 0 0.4 0.4 {}
T {7 mV} 1920 -110 0 0 0.4 0.4 {}
T {450 uW} 1920 -70 0 0 0.4 0.4 {}
T {VDD} 1750 -30 0 0 0.35 0.35 {}
T {1.8 V} 1920 -10 2 1 0.4 0.4 {}
T {Cp} 1750 10 0 0 0.35 0.35 {}
T {10p F} 1920 30 2 1 0.4 0.4 {}
T {Based this paper:
A HIGHLY LINEAR CASCODE-DRIVER
CMOS SOURCE-FOLLOWER BUFFER} -700 -1130 0 0 1 1 {}
N -650 -300 -550 -300 {
lab=#net1}
N -690 -270 -690 -180 {
lab=#net1}
N -510 -270 -510 -180 {
lab=#net2}
N -650 -150 -550 -150 {
lab=#net2}
N -600 -300 -600 -240 {
lab=#net1}
N -690 -240 -600 -240 {
lab=#net1}
N -600 -210 -600 -150 {
lab=#net2}
N -600 -210 -510 -210 {
lab=#net2}
N -650 -30 -550 -30 {
lab=V1}
N -690 -120 -690 -60 {
lab=#net3}
N -510 -120 -510 -60 {
lab=V1}
N -600 -90 -600 -30 {
lab=V1}
N -600 -90 -510 -90 {
lab=V1}
N -690 0 -690 40 {
lab=#net4}
N -510 0 -510 50 {
lab=VSS}
N -690 100 -690 140 {
lab=VSS}
N -730 70 -710 70 {
lab=VSS}
N -720 -150 -690 -150 {
lab=VSS}
N -720 -30 -690 -30 {
lab=VSS}
N -510 -150 -480 -150 {
lab=VSS}
N -510 -30 -480 -30 {
lab=VSS}
N -510 50 -510 140 {
lab=VSS}
N -370 -770 -300 -770 {
lab=VDD}
N -370 -730 -300 -730 {
lab=VSS}
N -370 -690 -300 -690 {
lab=VIN-}
N -370 -650 -300 -650 {
lab=VIN+}
N -370 -610 -300 -610 {
lab=VOUT}
N -730 70 -730 140 {
lab=VSS}
N -730 140 -680 140 {
lab=VSS}
N -680 140 -460 140 {
lab=VSS}
N -100 -20 -70 -20 {
lab=V1}
N -460 140 -30 140 {
lab=VSS}
N -30 10 -30 140 {
lab=VSS}
N -140 -100 -30 -100 {
lab=#net5}
N -30 -100 -30 -50 {
lab=#net5}
N -30 -100 80 -100 {
lab=#net5}
N -140 -130 80 -130 {
lab=VSS}
N -220 -130 -180 -130 {
lab=VIN+}
N 120 -130 160 -130 {
lab=VIN-}
N -140 -270 -140 -160 {
lab=#net6}
N 80 -270 80 -160 {
lab=#net7}
N -150 -350 90 -350 {
lab=VDD}
N -30 -20 30 -20 {
lab=VSS}
N 30 -20 30 30 {
lab=VSS}
N -30 30 30 30 {
lab=VSS}
N -170 -300 -140 -300 {
lab=VDD}
N 80 -300 110 -300 {
lab=VDD}
N 110 -330 110 -300 {
lab=VDD}
N -100 -300 40 -300 {
lab=#net6}
N -30 -300 -30 -240 {
lab=#net6}
N -140 -240 -30 -240 {
lab=#net6}
N 80 -350 80 -330 {
lab=VDD}
N 90 -350 110 -350 {
lab=VDD}
N 110 -350 110 -330 {
lab=VDD}
N -140 -350 -140 -330 {
lab=VDD}
N -170 -350 -170 -300 {
lab=VDD}
N -170 -350 -150 -350 {
lab=VDD}
N -690 -350 -170 -350 {
lab=VDD}
N -690 -350 -690 -330 {
lab=VDD}
N -510 -350 -510 -330 {
lab=VDD}
N -510 -300 -500 -300 {
lab=VDD}
N -500 -350 -500 -300 {
lab=VDD}
N -700 -300 -690 -300 {
lab=VDD}
N -700 -350 -700 -300 {
lab=VDD}
N -700 -350 -690 -350 {
lab=VDD}
N -30 140 360 140 {
lab=VSS}
N 290 50 320 50 {
lab=V1}
N 360 50 420 50 {
lab=VSS}
N 420 50 420 100 {
lab=VSS}
N 360 100 420 100 {
lab=VSS}
N 360 80 360 140 {
lab=VSS}
N 320 -90 340 -90 {
lab=VSS}
N 360 -20 360 20 {
lab=VB1}
N 110 -350 360 -350 {
lab=VDD}
N 360 -350 360 -340 {
lab=VDD}
N 360 -340 360 -330 {
lab=VDD}
N 360 -300 370 -300 {
lab=VDD}
N 370 -350 370 -300 {
lab=VDD}
N 360 -350 370 -350 {
lab=VDD}
N 360 -270 360 -240 {
lab=#net8}
N 360 -210 370 -210 {
lab=VDD}
N 370 -300 370 -210 {
lab=VDD}
N 360 -180 360 -120 {
lab=VB2}
N 360 -60 360 -20 {
lab=VB1}
N 280 -210 320 -210 {
lab=VB1}
N 280 -210 280 -20 {
lab=VB1}
N 280 -20 360 -20 {
lab=VB1}
N 300 -300 320 -300 {
lab=VB2}
N 300 -300 300 -150 {
lab=VB2}
N 300 -150 360 -150 {
lab=VB2}
N 360 -350 640 -350 {
lab=VDD}
N 820 -350 820 -330 {
lab=VDD}
N 760 -300 780 -300 {
lab=VB2}
N 820 -270 820 -240 {
lab=#net9}
N 740 -210 780 -210 {
lab=VB1}
N 820 -210 850 -210 {
lab=VDD}
N 850 -350 850 -210 {
lab=VDD}
N 820 -350 850 -350 {
lab=VDD}
N 820 -300 850 -300 {
lab=VDD}
N 820 -180 820 -160 {
lab=#net10}
N 820 -130 850 -130 {
lab=#net10}
N 850 -170 850 -130 {
lab=#net10}
N 820 -170 850 -170 {
lab=#net10}
N 820 -100 820 -60 {
lab=VOUT}
N 780 -130 780 -80 {
lab=VOUT}
N 780 -80 820 -80 {
lab=VOUT}
N 820 0 820 30 {
lab=#net11}
N 360 140 640 140 {
lab=VSS}
N 820 90 820 140 {
lab=VSS}
N 820 30 820 60 {
lab=#net11}
N 740 -30 780 -30 {
lab=#net7}
N 740 -30 740 60 {
lab=#net7}
N 740 60 780 60 {
lab=#net7}
N 820 -30 850 -30 {
lab=#net10}
N 850 -130 850 -30 {
lab=#net10}
N 80 -210 220 -210 {
lab=#net7}
N 220 -210 220 0 {
lab=#net7}
N 220 0 560 -0 {
lab=#net7}
N 640 140 820 140 {
lab=VSS}
N 560 0 740 -0 {
lab=#net7}
N 640 -350 820 -350 {
lab=VDD}
N 470 -60 490 -60 {
lab=#net7}
N 470 -60 470 -0 {
lab=#net7}
N 520 -40 520 140 {
lab=VSS}
N 550 -60 580 -60 {
lab=#net12}
N 640 -60 820 -60 {
lab=VOUT}
C {devices/lab_pin.sym} -590 140 3 0 {name=p3 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} -720 -150 0 0 {name=p8 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} -480 -150 0 1 {name=p9 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} -720 -30 0 0 {name=p10 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} -480 -30 0 1 {name=p11 sig_type=std_logic lab=VSS}
C {sky130_fd_pr/res_xhigh_po_0p35.sym} -690 70 0 0 {name=R10
W=0.35
L=1.2
model=res_xhigh_po_0p35
spiceprefix=X
 mult=2}
C {sky130_fd_pr/nfet_01v8_nf.sym} -670 -150 0 1 {name=M4
L=3
W=2
nf=2 mult=4
model=nfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8_nf.sym} -530 -150 0 0 {name=M3
L=3
W=2
nf=2 mult=4
model=nfet_01v8
spiceprefix=X
}
C {devices/iopin.sym} -370 -770 0 1 {name=p1 lab=VDD}
C {devices/lab_pin.sym} -300 -770 2 0 {name=p4 sig_type=std_logic lab=VDD}
C {devices/iopin.sym} -370 -730 0 1 {name=p6 lab=VSS}
C {devices/lab_pin.sym} -300 -730 2 0 {name=p7 sig_type=std_logic lab=VSS}
C {devices/iopin.sym} -370 -690 0 1 {name=p12 lab=VIN-}
C {devices/lab_pin.sym} -300 -690 2 0 {name=p13 sig_type=std_logic lab=VIN-}
C {devices/iopin.sym} -370 -650 0 1 {name=p17 lab=VIN+}
C {devices/lab_pin.sym} -300 -650 2 0 {name=p18 sig_type=std_logic lab=VIN+}
C {devices/iopin.sym} -370 -610 0 1 {name=p19 lab=VOUT}
C {devices/lab_pin.sym} -300 -610 2 0 {name=p20 sig_type=std_logic lab=VOUT}
C {devices/ngspice_get_value.sym} -740 -260 0 1 {name=r14 node=v(@m.$\{path\}xm1.msky130_fd_pr__pfet_01v8_lvt[vth])
descr="vth="}
C {devices/ngspice_get_value.sym} -740 -290 0 1 {name=r15 node=@m.$\{path\}xm1.msky130_fd_pr__pfet_01v8_lvt[gds]
descr="gds="}
C {devices/ngspice_get_value.sym} -740 -330 0 1 {name=r11 node=@m.$\{path\}xm1.msky130_fd_pr__pfet_01v8_lvt[gmbs]
descr="gmbs="}
C {devices/ngspice_get_value.sym} -440 -260 0 0 {name=r1 node=v(@m.$\{path\}xm2.msky130_fd_pr__pfet_01v8_lvt[vth])
descr="vth="}
C {devices/ngspice_get_value.sym} -440 -300 0 0 {name=r2 node=@m.$\{path\}xm2.msky130_fd_pr__pfet_01v8_lvt[gds]
descr="gds="}
C {devices/ngspice_get_value.sym} -440 -340 0 0 {name=r3 node=@m.$\{path\}xm2.msky130_fd_pr__pfet_01v8_lvt[gmbs]
descr="gmbs="}
C {devices/ngspice_get_value.sym} -770 -100 0 1 {name=r4 node=v(@m.$\{path\}xm4.msky130_fd_pr__nfet_01v8[vth])
descr="vth="}
C {devices/ngspice_get_value.sym} -770 -140 0 1 {name=r5 node=@m.$\{path\}xm4.msky130_fd_pr__nfet_01v8[gds]
descr="gds="}
C {devices/ngspice_get_value.sym} -770 -180 0 1 {name=r6 node=@m.$\{path\}xm4.msky130_fd_pr__nfet_01v8[gmbs]
descr="gmbs="}
C {devices/ngspice_get_value.sym} -440 -120 0 0 {name=r7 node=v(@m.$\{path\}xm3.msky130_fd_pr__nfet_01v8[vth])
descr="vth="}
C {devices/ngspice_get_value.sym} -440 -150 0 0 {name=r8 node=@m.$\{path\}xm3.msky130_fd_pr__nfet_01v8[gds]
descr="gds="}
C {devices/ngspice_get_value.sym} -440 -190 0 0 {name=r9 node=@m.$\{path\}xm3.msky130_fd_pr__nfet_01v8[gmbs]
descr="gmbs="}
C {sky130_fd_pr/nfet_01v8_nf.sym} -670 -30 0 1 {name=M6
L=2
W=3
nf=2 mult=8
model=nfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8_nf.sym} -530 -30 0 0 {name=M5
L=2
W=3
nf=2 mult=2
model=nfet_01v8
spiceprefix=X
}
C {devices/ngspice_get_value.sym} -770 30 0 1 {name=r12 node=v(@m.$\{path\}xm6.msky130_fd_pr__nfet_01v8[vth])
descr="vth="}
C {devices/ngspice_get_value.sym} -770 -10 0 1 {name=r13 node=@m.$\{path\}xm6.msky130_fd_pr__nfet_01v8[gds]
descr="gds="}
C {devices/ngspice_get_value.sym} -770 -50 0 1 {name=r16 node=@m.$\{path\}xm6.msky130_fd_pr__nfet_01v8[gmbs]
descr="gmbs="}
C {devices/ngspice_get_value.sym} -430 10 0 0 {name=r17 node=v(@m.$\{path\}xm5.msky130_fd_pr__nfet_01v8[vth])
descr="vth="}
C {devices/ngspice_get_value.sym} -430 -30 0 0 {name=r18 node=@m.$\{path\}xm5.msky130_fd_pr__nfet_01v8[gds]
descr="gds="}
C {devices/ngspice_get_value.sym} -430 -70 0 0 {name=r19 node=@m.$\{path\}xm5.msky130_fd_pr__nfet_01v8[gmbs]
descr="gmbs="}
C {devices/ngspice_get_value.sym} -810 -300 0 1 {name=r20 node=v(@m.$\{path\}xm1.msky130_fd_pr__pfet_01v8_lvt[vds])
descr="vds="}
C {devices/ngspice_get_value.sym} -810 -270 0 1 {name=r21 node=v(@m.$\{path\}xm1.msky130_fd_pr__pfet_01v8_lvt[vgs])
descr="vgs="}
C {devices/ngspice_get_value.sym} -370 -310 0 1 {name=r22 node=v(@m.$\{path\}xm2.msky130_fd_pr__pfet_01v8_lvt[vds])
descr="vds="}
C {devices/ngspice_get_value.sym} -370 -280 0 1 {name=r23 node=v(@m.$\{path\}xm2.msky130_fd_pr__pfet_01v8_lvt[vgs])
descr="vgs="}
C {devices/ngspice_get_value.sym} -850 -140 0 1 {name=r24 node=v(@m.$\{path\}xm4.msky130_fd_pr__nfet_01v8[vds])
descr="vds="}
C {devices/ngspice_get_value.sym} -850 -110 0 1 {name=r25 node=v(@m.$\{path\}xm4.msky130_fd_pr__nfet_01v8[vgs])
descr="vgs="}
C {devices/ngspice_get_value.sym} -390 -170 0 0 {name=r26 node=v(@m.$\{path\}xm3.msky130_fd_pr__nfet_01v8[vds])
descr="vds="}
C {devices/ngspice_get_value.sym} -390 -140 0 0 {name=r27 node=v(@m.$\{path\}xm3.msky130_fd_pr__nfet_01v8[vgs])
descr="vgs="}
C {devices/ngspice_get_value.sym} -830 -10 0 1 {name=r28 node=v(@m.$\{path\}xm6.msky130_fd_pr__nfet_01v8[vds])
descr="vds="}
C {devices/ngspice_get_value.sym} -830 20 0 1 {name=r29 node=v(@m.$\{path\}xm6.msky130_fd_pr__nfet_01v8[vgs])
descr="vgs="}
C {devices/ngspice_get_value.sym} -380 -40 0 0 {name=r30 node=v(@m.$\{path\}xm5.msky130_fd_pr__nfet_01v8[vds])
descr="vds="}
C {devices/ngspice_get_value.sym} -380 -10 0 0 {name=r31 node=v(@m.$\{path\}xm5.msky130_fd_pr__nfet_01v8[vgs])
descr="vgs="}
C {sky130_fd_pr/pfet_01v8_lvt_nf.sym} -530 -300 0 0 {name=M2
L=3
W=4
nf=2 mult=2
model=pfet_01v8_lvt
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8_lvt_nf.sym} -670 -300 0 1 {name=M1
L=3
W=4
nf=2 mult=2
model=pfet_01v8_lvt
spiceprefix=X
}
C {devices/title-3.sym} -1060 340 0 0 {name=l2 author="Vasco Luz" rev=1.0 lock=false}
C {sky130_fd_pr/nfet_01v8_nf.sym} -50 -20 0 0 {name=M7
L=2
W=3
nf=2 mult=24
model=nfet_01v8
spiceprefix=X
}
C {devices/lab_pin.sym} -590 -90 3 1 {name=p5 sig_type=std_logic lab=V1}
C {devices/lab_pin.sym} -100 -20 3 1 {name=p14 sig_type=std_logic lab=V1}
C {sky130_fd_pr/nfet_01v8_nf.sym} -160 -130 0 0 {name=M34
L=1
W=6  
nf=4 mult=4
model=nfet_01v8
spiceprefix=X
}
C {devices/lab_pin.sym} -30 -130 1 0 {name=p15 sig_type=std_logic lab=VSS}
C {sky130_fd_pr/nfet_01v8_nf.sym} 100 -130 0 1 {name=M8
L=1
W=6  
nf=4 mult=4
model=nfet_01v8
spiceprefix=X
}
C {devices/lab_pin.sym} -220 -130 2 1 {name=p16 sig_type=std_logic lab=VIN+}
C {devices/lab_pin.sym} 160 -130 2 0 {name=p21 sig_type=std_logic lab=VIN-}
C {sky130_fd_pr/pfet_01v8_lvt_nf.sym} -120 -300 0 1 {name=M9
L=1
W=3
nf=2 mult=2
model=pfet_01v8_lvt
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8_lvt_nf.sym} 60 -300 0 0 {name=M10
L=1
W=3
nf=2 mult=2
model=pfet_01v8_lvt
spiceprefix=X
}
C {devices/lab_pin.sym} -290 -350 1 0 {name=p2 sig_type=std_logic lab=VDD}
C {sky130_fd_pr/nfet_01v8_nf.sym} 340 50 0 0 {name=M11
L=2
W=3
nf=2 mult=12
model=nfet_01v8
spiceprefix=X
}
C {devices/lab_pin.sym} 290 50 3 1 {name=p22 sig_type=std_logic lab=V1}
C {sky130_fd_pr/res_xhigh_po_0p35.sym} 360 -90 0 0 {name=R32
W=0.35
L=2
model=res_xhigh_po_0p35
spiceprefix=X
 mult=2}
C {sky130_fd_pr/pfet_01v8_lvt_nf.sym} 340 -210 0 0 {name=M13
L=1
W=4
nf=4 mult=2
model=pfet_01v8_lvt
spiceprefix=X
}
C {devices/lab_pin.sym} 320 -90 3 0 {name=p23 sig_type=std_logic lab=VSS}
C {devices/ngspice_get_value.sym} 460 -190 0 1 {name=r36 node=v(@m.$\{path\}xm13.msky130_fd_pr__pfet_01v8_lvt[vds])
descr="vds="}
C {devices/ngspice_get_value.sym} 460 -230 0 1 {name=r38 node=v(@m.$\{path\}xm13.msky130_fd_pr__pfet_01v8_lvt[vgs])
descr="vgs="}
C {devices/lab_pin.sym} 300 -300 3 1 {name=p24 sig_type=std_logic lab=VB2}
C {devices/lab_pin.sym} 280 -210 3 1 {name=p25 sig_type=std_logic lab=VB1}
C {devices/ngspice_get_value.sym} 420 -160 0 0 {name=r37 node=v(@m.$\{path\}xm13.msky130_fd_pr__pfet_01v8_lvt[vth])
descr="vth="}
C {sky130_fd_pr/pfet_01v8_lvt_nf.sym} 340 -300 0 0 {name=M12
L=1
W=4
nf=4 mult=2
model=pfet_01v8_lvt
spiceprefix=X
}
C {devices/ngspice_get_value.sym} 470 -340 0 1 {name=r33 node=v(@m.$\{path\}xm12.msky130_fd_pr__pfet_01v8_lvt[vgs])
descr="vgs="}
C {devices/ngspice_get_value.sym} 460 -300 0 1 {name=r34 node=v(@m.$\{path\}xm12.msky130_fd_pr__pfet_01v8_lvt[vds])
descr="vds="}
C {devices/ngspice_get_value.sym} 420 -270 0 0 {name=r35 node=v(@m.$\{path\}xm12.msky130_fd_pr__pfet_01v8_lvt[vth])
descr="vth="}
C {sky130_fd_pr/pfet_01v8_lvt_nf.sym} 800 -300 0 0 {name=M14
L=1
W=4
nf=4 mult=16
model=pfet_01v8_lvt
spiceprefix=X
}
C {devices/lab_pin.sym} 760 -300 3 1 {name=p26 sig_type=std_logic lab=VB2}
C {sky130_fd_pr/pfet_01v8_lvt_nf.sym} 800 -210 0 0 {name=M15
L=1
W=4
nf=4 mult=16
model=pfet_01v8_lvt
spiceprefix=X
}
C {devices/lab_pin.sym} 740 -210 3 1 {name=p27 sig_type=std_logic lab=VB1}
C {devices/lab_pin.sym} 820 -90 2 0 {name=p28 sig_type=std_logic lab=VOUT}
C {sky130_fd_pr/pfet_01v8_lvt_nf.sym} 800 -30 0 0 {name=M17
L=0.35
W=6
nf=20 mult=4
model=pfet_01v8_lvt
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8_lvt_nf.sym} 800 60 0 0 {name=M18
L=0.35
W=6
nf=20 mult=4
model=pfet_01v8_lvt
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8_lvt_nf.sym} 800 -130 0 0 {name=M16
L=0.35
W=6
nf=20 mult=4
model=pfet_01v8_lvt
spiceprefix=X
}
C {sky130_fd_pr/res_xhigh_po_0p35.sym} 520 -60 3 0 {name=R39
W=0.35
L=0.4
model=res_xhigh_po_0p35
spiceprefix=X
 mult=2}
C {sky130_fd_pr/cap_mim_m3_2.sym} 610 -60 1 0 {name=C1 model=cap_mim_m3_2 W=9.6 L=9.6 MF=20 spiceprefix=X}
